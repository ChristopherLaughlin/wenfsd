...
2. **Apply for Tesla Fleet API access** at https://developer.tesla.com — *start this first, it
   ...
   `https://wenfsd.info/auth/callback`, request scope `vehicle_device_data`. (Details:
...
4. **Set env vars** (in the host's dashboard): `MOCK_MODE=false`, `PUBLIC_BASE_URL=https://wenfsd.info`,
   `TESLA_CLIENT_ID`, `TESLA_CLIENT_SECRET`, `TESLA_REDIRECT_URI`. `SESSION_SECRET` and
   `DATABASE_URL` are generated for you.

5. **Point the domains** — `wenfsd.info` → the service (add it as a custom domain in Railway/Render,
   set the DNS record they give you). `wenfsd.com` (also yours) can redirect to `.info`.
